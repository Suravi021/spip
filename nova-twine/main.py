...
class MyCLI(cmd.Cmd):
    ...
    def ExistenceChecker(self):
        ...
        if not os.path.exists(self.directory_path):
            ...
            return False
        return True
    def ReputationCheck(self, owner, repo, token=None):
        owner = self.username
        if self.ExistenceChecker():
    ...
    def get_github_commits(self, owner, repo, token=None):
        ...
        if response.status_code == 200:
            ...
            githubrep = self.ReputationCheck(owner=self.username,repo=repo)
            githuburl = repo2
            ...
            fileHandle = open(self.description_path, 'r')
    ...
    def do_start(self, line):
        ...
        self.pattern = r'[^/]*$'
        self.match = re.search(self.pattern, self.github)
        self.repo = self.match.group(0)


    def do_upload(self, line):
        data = self.get_github_commits(self.username, self.repo)
